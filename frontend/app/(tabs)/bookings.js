...
import { Calendar, MapPin, Ticket, Trash2, Sparkles, Edit3 } from 'lucide-react-native';
          ...
          <View className="mx-6 mb-10 overflow-hidden rounded-[48px] border border-white/5 bg-white/[0.01] p-10 shadow-2xl">
            ...
            <View className="flex-row items-center justify-between mb-4">
              ...
              <Text className={`text-[10px] font-black uppercase tracking-widest ${item.status !== 'cancelled' ? 'text-cyan-400' : 'text-slate-500'}`}>
                {item.status !== 'cancelled' ? 'Active' : 'Archived'}
              </Text>
            </View>

            <View className="flex-row gap-4">
              <TouchableOpacity
                disabled={!item.can_modify || cancelMutation.isPending}
                onPress={() => router.push({
                  pathname: `/booking/${item.show_id}`,
                  params: { reservationId: item.id }
                })}
                className={`h-14 flex-1 flex-row items-center justify-center rounded-2xl border ${
                  item.can_modify
                    ? 'border-cyan-500/30 bg-cyan-500/10'
                    : 'border-white/5 bg-white/5 opacity-40'
                }`}
              >
                <Edit3 size={18} color={item.can_modify ? Colors.primary : '#64748b'} />
                <Text className={`ml-3 text-sm font-black uppercase tracking-[0.2em] ${
                  item.can_modify ? 'text-cyan-300' : 'text-slate-500'
                }`}>
                  Modify Slot
                </Text>
              </TouchableOpacity>
